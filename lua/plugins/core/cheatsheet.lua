...
-- Caches the data after first generation for instant subsequent opens
local _cheatsheet_cache = nil

local function get_cheatsheet_data()
    -- Return cached data if available
    if _cheatsheet_cache then
        return _cheatsheet_cache
    end

    -- Generate cheatsheet data (only runs once per session)
    ...
    _cheatsheet_cache = cheatsheet_data
                        ...
                        .new({}, {
                            ...
                            attach_mappings = function(prompt_bufnr, map)
                                actions.select_default:replace(function()
                                    ...
                                    if selection then
                                        vim.notify(
                                            string.format('%s: %s\n%s', selection.value.category, selection.value.key,
                                    ...
                                    if subgroup_count > 0 then
                                        ...
                                        for _, cat in ipairs(subgroup_categories) do
                                            table.insert(display_list, {
                                                ...
                                                display_name = string.format('    │  ├─ %-30s [%d]', cat,
                                                    category_counts[cat] or 0),
                            ...
                            .new({}, {
                                        ...
                                        if item.type == 'group' then
                                            ...
                                            for _, list_item in ipairs(display_list) do
                                                if list_item.type == 'subgroup' and list_item.parent_group == item.name then
                                                    table.insert(lines,
                                                        string.format('  • %-30s [%d keymaps]', list_item.name,
                                                            list_item.count))
                                                end
                                            end
                                        ...
                                        elseif item.type == 'category' then
                                            ...
                                            for _, cs_item in ipairs(cheatsheet) do
                                                if cs_item.category == item.name and count < 10 then
                                                    table.insert(lines,
                                                        string.format('  %-25s  %s', cs_item.key, cs_item.desc))
                                                    count = count + 1
                                                end
                                            end
                                            ...
                                            if item.count > 10 then
                                                table.insert(lines, '')
                                                table.insert(lines,
                                                    string.format('  ... and %d more keymaps', item.count - 10))
                                            end
                                        end
                                ...
                                attach_mappings = function(prompt_bufnr)
                                    actions.select_default:replace(function()
                                        ...
                                        if item.type == 'separator' then
                                            vim.notify('Cannot select empty lines - choose a category or group',
                                                vim.log.levels.INFO)
                                            return
                                        end
